...
@dashboard.route('/themes/manage')
@login_required
def manage_themes():
    themes = Blog_Theme.query.all()
    return render_template('dashboard/manage_themes.html', themes=themes, logged_in=current_user.is_authenticated)

@dashboard.route('/themes/edit/<int:id>', methods=['GET', 'POST'])
@login_required
def edit_theme(id):
    theme = Blog_Theme.query.get_or_404(id)
    
    if request.method == "POST":
        theme.theme = request.form.get('theme_name')
        theme.picture_source = request.form.get('image_source')
        
        theme_image = request.files.get('theme_image')
        if theme_image:
            # Handle new image upload
            filename = secure_filename(theme_image.filename)
            unique_filename = f"theme_{str(uuid.uuid4())}_{filename}"
            
            # Save new image
            theme_image.save(os.path.join(current_app.config["THEME_IMG_FOLDER"], unique_filename))
            
            # Delete old image
            if theme.picture:
                old_image_path = os.path.join(current_app.config["THEME_IMG_FOLDER"], theme.picture)
                if os.path.exists(old_image_path):
                    os.remove(old_image_path)
            
            theme.picture = unique_filename
    ...
    return render_template('dashboard/edit_theme.html', theme=theme, logged_in=current_user.is_authenticated)
@dashboard.route('/themes/delete/<int:id>', methods=['POST'])
@login_required
def delete_theme(id):
    theme = Blog_Theme.query.get_or_404(id)
    
    try:
        # Delete theme image if it exists
        if theme.picture:
            image_path = os.path.join(current_app.config["THEME_IMG_FOLDER"], theme.picture)
    ...
    return redirect(url_for('dashboard.manage_themes'))
@dashboard.route('/themes/create', methods=['GET', 'POST'])
@login_required
def create_theme():
    if request.method == "POST":
        ...
        if theme_image:
            ...
            new_theme = Blog_Theme(
                theme=theme_name,
                picture=unique_filename,
                picture_source=image_source
            )
    ...
    return render_template('dashboard/create_theme.html', logged_in=current_user.is_authenticated)
